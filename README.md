...
> この`types-complete` branchはTypes Extraの復旧用checkpointです。通常のWorkshopは`main` branchから開始します。この状態を利用する場合は[Recovery Guide](docs/04-RECOVERY.md)の手順で必要なfileだけ復元してください。
...
├── examples/shippingconst/          # named constantを使うTypes Extraのexample
├── docs/                            # 仕組みとCIの補足資料
├── go.mod
└── README.md
```
