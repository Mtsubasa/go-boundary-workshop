...
> この`ex03-complete` branchはStep 1の復旧用checkpointです。通常のWorkshopは`main` branchから開始します。この状態を利用する場合は[Recovery Guide](docs/04-RECOVERY.md)の手順で必要なfileだけ復元してください。
...
│   ├── analyzer_workshop_test.go   # 完成したanalyzer用のtest
